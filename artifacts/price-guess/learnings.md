...
applied: not-yet
---
## LLM 폴백 데이터를 매 요청 생성 → 캐싱으로 해결

**상황**: 출시 후 "불러오는 시간이 너무 오래 걸려". 원인: KAMIS 키 부재로 매 문제(/api/quiz)마다 Gemini를 새로 호출(문항당 ~11s). 10문제 = 10회 LLM 호출.
**판단**: (1) Gemini 결과도 `writeCache`해서 첫 문제만 생성하고 나머지는 캐시 재사용 → 2번째부터 ~7ms. 단 캐시를 통과해도 출처를 잃지 않도록 `estimated`를 PriceData(데이터)에 실어 캐시에 함께 저장(이전엔 PriceResult 경로에서만 판단해 캐시 통과 시 유실됐을 것). (2) `gemini-2.5-flash`의 thinking을 `thinkingBudget:0`으로 끄고 `responseMimeType:"application/json"` 강제 → 콜드 11s→5s.
**다시 마주칠 가능성**: 높음 — LLM을 데이터 소스로 쓰면 "매 요청 호출"이 기본 함정. 교훈: LLM 응답은 캐시 가능한 데이터로 취급하고, 출처/메타는 파생값이 아니라 데이터에 실어 캐시·저장 경로에서 유실되지 않게.
